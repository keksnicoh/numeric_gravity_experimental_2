""" OpenGl gravity simulation n8

    context:
    - simulate massive amout of particles
      directly with numpy and render objects
      as singular dots by a geometry shader
      and a fragment shader.

      this approach allows live calculation of 200k+
      particles.

    basic euler approximation of newtons gravity.
    approximation:
    - linear steps with length p_dt
    - only heavy partices correlate
    - to avoid singularities in massive numpy
      matrix calculations, each distance will
      increased by 4th dim constant p_eps

    known issues:
    - singulaties when objects are close
    - no collision check
    - base world framework is quiet unflexible
...
def initContext(world):
	...
	print "configure 200k masses..."
	dphi = 2*pi / c_obj_n * 100
	factor = 3/float(c_obj_n)
	for obj in range(0,c_obj_n):
		sat_v = 1.5*sqrt(main_mass/(1.2*c_r));
	...
	p_vel[c_obj_n+1][0] = 0
	p_vel[c_obj_n+1][1] = -10
	p_vel[c_obj_n+1][2] = 40
	p_mass[c_obj_n+1] = main_mass
	...
	print "ready! press + to start simulation..."
